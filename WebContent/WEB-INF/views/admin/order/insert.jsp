...
<script type="text/javascript">
	$(document)
			.on(
					...
					function() {
						...
						$('#tPrice').text(
								Math.ceil(
										parseInt(uPrice.value)
												* parseInt(qty.value)).toFixed(
										0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g,
										"$1,")
										+ ' đ');
					});
	...
	$(document).on(
			...
			function() {
				$('#tPrice').text(
						Math.ceil(parseInt(uPrice.value) * parseInt(qty.value))
								.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g,
										"$1,")
								+ ' đ');
			});
		...
		<div id="page-wrapper">
			...
			<div id="insertItemForm">
				<form:form action="admin/order/insert/insertItem.html"
					id="frmInsert" modelAttribute="orderItem">
					<form:hidden path="id" />
